...
Supports Car Race files (.crsrc) files as Python fils
...
class CommandFile(Toplevel):
    ...
    def open(self, src_file=None):
        ...
        if path is None:
            path = SlTrace.getSourcePath(src_file + ".crsrc", report=False, req=False)
        if path is None:
            raise SelectError("open can't find %s(.py, .crsrc) in %s"
