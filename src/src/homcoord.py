...
class Xform(object):
    ...
    def inverse ( self ):
        ...
        #   else -> I ]
        if self._mInverse is None:
            self._mInverse = np.linalg.inv ( self._m )
    ...
    def compose ( self, t2 ):
        ...
        '''
        return Xform ( Xmult ( t2._m, self._m ) )
        ###ORIGreturn Xform ( dot ( t2._m, self._m ) )
...
def dot(a,b):
    ...
    try: #matrix*vector
        ...
        pass
    #matrix*matrix
    raise BaseException("Move matrix*matrix to Xmul")
...
def Xmult(m1, m2):
    """ Matrix 3 x 3 multiplication m1 x m2
    Pulled out of dot because:
     1. problem with zip / iteration
     2. matrix is a special case and probably can be optimized
    :m1: matrix 1
    :m2: matrix 2
    :returns: matrix product: m1 x m2
    """
    res=[dot(m1,col) for col in zip(*m2)]
    m3 = list(map(list, zip(*res)))
    return m3
